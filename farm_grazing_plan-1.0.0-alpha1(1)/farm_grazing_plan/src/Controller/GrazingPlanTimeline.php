...
class GrazingPlanTimeline extends ControllerBase {
  ...
  protected function buildTimeline(PlanInterface $plan, array $grazing_events_by_asset) {
    // Check state to see if overlapping logs need to be shifted.
    ...
    foreach ($grazing_events_by_asset as $asset_id => $grazing_events) {
      ...
      // Sort the grazing event rows by start time.
    ...
    // Serialize timeline data as JSON.
   ...
   *
   * Differentiates between planned (value 0 or not set) and actual (value 1)
   * events by generating tasks with distinct classes.
   *
  ...
  protected function buildGrazingEventRow(GrazingEventInterface $grazing_event) {
    ...
    // Start building the list of tasks.
    $tasks = [];
    ...
    // For planned events (0 or not assigned).
    if (0 == $grazing_event->get('planned')->value) {
      ...
      if (!empty($grazing_event->get('recovery')->value)) {
        $tasks[] = [
          ...
          'end' => $grazing_event->get('start')->value + ($grazing_event->get('duration')->value * 3600) + ($grazing_event->get('recovery')->value * 3600),
          'meta' => [
            'stage' => 'recovery',
          ],
          'classes' => [
    ...
    // Always include a task for editing the associated movement log.
   ...
   *
   * @return array
   *   An array representing a single log task.
   */
   ...
   *
   * Iterates through each asset's events, and if an event starts before or
   * during the previous event's run (including recovery), its start time is
   * adjusted.
   *
   * @param array $grazing_events_by_asset
